...
export default function Event() {
  ...
  function getEvent(eventID) {
    axios
      ...
      .then((response) => {
        ...
        setSelectedEventBuffer(JSON.parse(response.data.result.result[0].buffer_time));
      })
      .catch((error) => {
        console.log(error);
      });
    ...
    return (
      ...
      >
        ...
        <Modal.Body style={bodyStyle}>
          ...
          />
          <Typography className={classes.colHeader}> Buffer time before or after events </Typography>
          <div>
            <input
              type="checkbox"
              id="before"
              checked={selectedEventBuffer.before.is_enabled !== "False" && selectedEventBuffer.before.is_enabled !== false}
              onChange={(event) => {
          ...
          />
          <div>
            {console.log(selectedEventBuffer.before.time, selectedEventBuffer.after.time)}
            <input
              type="checkbox"
              checked={selectedEventBuffer.after.is_enabled !== "False" && selectedEventBuffer.after.is_enabled !== false}
              onChange={(event) => {
  ...
  function createNewEvent(viewID) {
    ...
    let at = "";
    let d = "";
    const numHoursBT = beforeBufferTime >= 60 ? Math.floor(beforeBufferTime / 60) : "00";
    let numMinsBT = beforeBufferTime % 60;
    if (numMinsBT < 10) numMinsBT = "0" + numMinsBT;
    bt = `${numHoursBT}:${numMinsBT}:00`;

    const numHoursAT = afterBufferTime >= 60 ? Math.floor(afterBufferTime / 60) : "00";
    let numMinsAT = afterBufferTime % 60;
    if (numMinsAT < 10) numMinsAT = "0" + numMinsAT;
    at = `${numHoursAT}:${numMinsAT}:00`;

    const numHoursD = eventDuration >= 60 ? Math.floor(eventDuration / 60) : "00";
  ...
  const createNewEventModal = () => {
    ...
    return (
      <Modal show={showCreateNewEventModal} onHide={closeCreateNewEventModal} style={modalStyle} backdropClassName="modal-backdrop foo-modal-backdrop in">
        <Modal.Header style={headerStyle} closeButton>
          <Modal.Title>
            <Typography
              style={{
                  ...
                  <div>
                    {event.view_id === view.view_unique_id ? (
                      <div>
                        ...
                        >
                          <Col style={{ cursor: "pointer" }} onClick={() => navigator.clipboard.writeText(document.location.href + "/" + `${event.event_unique_id}`)}>
