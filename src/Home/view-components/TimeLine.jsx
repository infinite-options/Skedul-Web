...
const TimeLine = (props) => {
  ...
  const { direction = "horizontal", type = "all", pixelSize = 750, label = "Sunday", setSlotsData } = props;
  ...
  class TimeSlotData {
    ...
    getStyle() {
      return {
        ...
        MsUserSelect: "none",
        userSelect: "none",
        transition: "left 0.05s ease, top 0.05s ease, height 0.05s ease,width 0.05s ease",
      };
    }
    ...
    setCellDistFromBoundingClient(etarget) {
      this.cellDistFromBoundingClient = this.direction === "vertical" ? etarget.getBoundingClientRect().top : etarget.getBoundingClientRect().left;
    }
    setRawPosition(e) {
      this.rawPosition = this.direction === "vertical" ? e.clientY - this.cellDistFromBoundingClient : e.clientX - this.cellDistFromBoundingClient;
    }
    setThirtyMinSize(etarget) {
      this.thirtyMinSize =
        this.direction === "vertical" ? (etarget.parentElement.parentElement.clientHeight - 100) / 48 : (etarget.parentElement.parentElement.clientWidth - 100) / 48;
    }
    setTime(isFor = "pos", time) {
      if (isFor === "pos") {
        this.time = {
          start_time: `${getTime(this.rawPosition - (this.rawPosition % this.thirtyMinSize), this.thirtyMinSize)}`,
          end_time: `${getTime(this.rawPosition - (this.rawPosition % this.thirtyMinSize), this.thirtyMinSize)}`,
        };
      } else if (isFor === "size") {
        this.time.end_time = `${getTime(
          this.rawPosition + this.thirtyMinSize / 5 - ((this.rawPosition + this.thirtyMinSize / 5) % this.thirtyMinSize) - this.position >= 0
            ? this.rawPosition + this.thirtyMinSize / 5 - ((this.rawPosition + this.thirtyMinSize / 5) % this.thirtyMinSize)
            : 0,
          this.thirtyMinSize
        )}`;
      } else if (isFor === "time") {
    ...
    correctOverlapsForSelected(timeSlotsData) {
      ...
      data.forEach((timeSlot, idx) => {
        ...
        let checks = {
          thisEndIsBetweenTimeSlot: this.position + this.size > timeSlot.position && this.position + this.size <= timeSlot.position + timeSlot.size,
          thisStartIsBetweenTimeSlot: this.position >= timeSlot.position && this.position < timeSlot.position + timeSlot.size,
        ...
        if (checks.thisEndIsBetweenTimeSlot && checks.thisStartIsBetweenTimeSlot) {
          ...
          timeSlot.time = {
            ...
            end_time: `${getTime(timeSlot.position + timeSlot.size, this.thirtyMinSize)}`,
          };
          data = [
            ...data.flatMap((t, i) => {
              if (idx === i) {
                let splitSlot = new TimeSlotData(timeSlot.direction, timeSlot.label, timeSlot.color, time);
                splitSlot.cellDistFromBoundingClient = timeSlot.cellDistFromBoundingClient;
          ...
          data[idx + 1].size = data[idx + 1].position + data[idx + 1].size - (this.position + this.size);
          data[idx + 1].position = this.position + this.size;
          data[idx + 1].time = {
            start_time: `${getTime(data[idx + 1].position, this.thirtyMinSize)}`,
            end_time: `${getTime(data[idx + 1].position + data[idx + 1].size, this.thirtyMinSize)}`,
          };
          return;
        }
        ...
        if (checks.thisEndIsBetweenTimeSlot) {
          ...
          timeSlot.size = timeSlot.position + timeSlot.size - (this.position + this.size);
          timeSlot.position = this.position + this.size;
          timeSlot.time = {
            start_time: `${getTime(timeSlot.position, this.thirtyMinSize)}`,
            end_time: `${getTime(timeSlot.position + timeSlot.size, this.thirtyMinSize)}`,
    ...
    correctOverlapsForAll(timeSlotsData) {
      ...
      data.forEach((timeSlot, idx) => {
        ...
        if (checks.thisStartIsBetweenTimeSlot) {
          ...
          let opaqueStart = 0;
          let opaqueEnd = ((timeSlot.position + timeSlot.size - this.position) / this.size) * 100;
          opaqueParts.push({
            percentages: [opaqueStart, opaqueEnd],
            str: `, ${color} ${opaqueStart}%, ${opaqueColor} ${opaqueStart}%, ${opaqueColor} ${opaqueEnd}%, ${color} ${opaqueEnd}%`,
          });
        ...
        if (checks.thisEndIsBetweenTimeSlot) {
          ...
          // this.position + this.size --- 100%
          let opaqueStart = ((timeSlot.position - this.position) / this.size) * 100;
          let opaqueEnd = 100;
          opaqueParts.push({
            percentages: [opaqueStart, opaqueEnd],
            str: `, ${color} ${opaqueStart}%, ${opaqueColor} ${opaqueStart}%, ${opaqueColor} ${opaqueEnd}%, ${color} ${opaqueEnd}%`,
    ...
    IsNearThirtyMinMark() {
      return this.rawPosition % this.thirtyMinSize < this.thirtyMinSize / 5 || this.rawPosition % this.thirtyMinSize > this.thirtyMinSize - this.thirtyMinSize / 5 ? true : false;
    }
  }
  ...
  const handleOnMouseDown = (e, timeSlotsData, allViews, cell, direction, label) => {
    // Set new timeSlot
    let color = null;
  ...
  const setSelectedViewTimeSlots = (timeSlotsData, allViews, label, direction, cell) => {
    let data = timeSlotsData.current;
    ...
    if (schedule.length > 0) {
      schedule.forEach((slot) => {
        data.push(new TimeSlotData(direction, label, selectedView.color ? selectedView.color : "#FFFFFF", slot));
  ...
  const setAllViewsTimeSlots = (timeSlotsData, allViews, label, direction, cell) => {
    let data = timeSlotsData.current;
    ...
    allViews.forEach((view) => {
      ...
      if (schedule[label].length > 0) {
        schedule[label].forEach((slot) => {
          data.push(new TimeSlotData(direction, label, view.color ? view.color : "#FFFFFF", slot));
          data[data.length - 1].setCellDistFromBoundingClient(cell.current);
          data[data.length - 1].setThirtyMinSize(cell.current);
          data[data.length - 1].setPosition();
          data[data.length - 1].setSize();
  ...
  useEffect(() => {
    ...
    if (type === "selected" && allViews.length > 0) {
      timeSlotsData.current = setSelectedViewTimeSlots(timeSlotsData, allViews, label, direction, cell);
    } else if (type === "all" && allViews.length > 0) {
      timeSlotsData.current = setAllViewsTimeSlots(timeSlotsData, allViews, label, direction, cell);
    }
    timeSlotsData.current = handleOnMouseUpOnMouseLeaveOnPopoverClose(timeSlotsData);
  ...
  const handleAddClick = (e, timeSlotsData, allViews, label, direction, cell, input, setAnchorEl) => {
  ...
  return (
    <div
      style={{
        ...
        display: "grid",
        alignContent: "center",
        gridTemplateColumns: direction === "horizontal" ? `60px ${pixelSize} 40px` : "",
        gridTemplateRows: direction === "vertical" ? `60px ${pixelSize} 40px` : "",
        flexGrow: "1",
    ...
    >
      ...
      <div
        ...
        onMouseDownCapture={(e) => {
          if (type === "selected" && allViews.length > 0) {
            e.persist();
            timeSlotsData.current = handleOnMouseDown(e, timeSlotsData, allViews, cell, direction, label);
          }
          setTimeSlots([
            ...timeSlotsData.current.map((timeSlot, idx) => (
              <div
      ...
      {type === "selected" && (
        ...
        >
          <IconButton
            ...
            onClick={(e) => {
              if (type === "selected" && allViews.length > 0) {
                timeSlotsData.current = handleAddClick(e, timeSlotsData, allViews, label, direction, cell, input, setAnchorEl);
                timeSlotsData.current = handleOnMouseUpOnMouseLeaveOnPopoverClose(timeSlotsData);
              }
              setTimeSlots([
                ...timeSlotsData.current.map((timeSlot, idx) => (
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  if (type === "selected" && allViews.length > 0 && e.target.value !== "") {
                    timeSlotsData.current = handleTimeChange(input, timeSlotsData);
